...
def compress_command(
    ...
    dpi: int = typer.Option(150, "--dpi", "-d", help="DPI for compression (default: 150)"),
    target_size: Optional[float] = typer.Option(None, "--target-size", "-s", help="Target size in MB (auto mode)"),
    ...
    def logger(message: str):
        if not quiet:
            ...
            clean_message = clean_message.replace("[bold yellow]", "").replace("[/bold yellow]", "")
            ...
            clean_message = clean_message.replace("[bold cyan]", "").replace("[/bold cyan]", "")
...
def convert_command(
    input_file: Path = typer.Argument(..., help="Input PDF file path"),
    output_file: Optional[Path] = typer.Option(None, "--output", "-o", help="Output PowerPoint file path"),
...
def info_command(
    ...
    try:
        ...
        info_text.append(f"Dimensions: {width_pt:.1f} × {height_pt:.1f} pt ({width_in:.1f}\" × {height_in:.1f}\")\n")
